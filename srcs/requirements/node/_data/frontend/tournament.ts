...
import { saveMatchHistory } from "./logout.js";
...
export function tournamentLogic(game: GameInfo): number {
	...
	if (game.t.matches[length - 1].player1.score === rounds ||
		...
		saveMatchHistory({  										// save individual Match_history
			player1_alias: game.t.matches[length - 1].player1.name,
			player2_alias: game.t.matches[length - 1].player2.name,
			winner_alias: game.t.matches[length - 1].winner.name,
			loser_alias: game.t.matches[length - 1].loser.name,
			score_player1: game.t.matches[length - 1].player1.score,
			score_player2: game.t.matches[length - 1].player2.score,
